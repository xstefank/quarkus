...
public class ResteasyReactiveServerLoggingFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        sb.append(requestContext.getMethod()).append(" ")
                .append(sanitize(requestContext.getUriInfo().getRequestUri().toString()));
        ...
        if (logging.includeBody()) {
            if (requestContext.hasEntity()) {
                ...
                if (isMultipartMediaType(mediaType)) {
                    // Multipart needs full bytes to locate part boundaries
        ...
        if (routingContext != null) {
            routingContext.addHeadersEndHandler(v -> {
                ...
                rsb.append("Status[").append(routingContext.response().getStatusCode())
                        .append(" ").append(routingContext.response().getStatusMessage()).append("], ");
    ...
    public void filter(ContainerRequestContext requestContext, ContainerResponseContext responseContext) {
        ...
        sb.append("Status[").append(responseContext.getStatus())
                .append(" ").append(responseContext.getStatusInfo().getReasonPhrase()).append("], ");
        ...
        if (logging.includeBody()) {
            ...
            if (entity instanceof MultipartFormDataOutput) {
                ...
                if (isMultipartMediaType(responseContext.getMediaType())) {
                    // @RestForm POJO response — use the build-time generated mapper
                    MultipartFormDataOutput formData = tryMapToMultipart(entity);
                    if (formData != null) {
    ...
    private MultipartFormDataOutput tryMapToMultipart(Object entity) {
        // The generated class name follows the convention established in MultipartMessageBodyWriter
        String mapperClassName = entity.getClass().getName() + "_generated_mapper";
        try {
            Class<?> mapperClass = Thread.currentThread().getContextClassLoader().loadClass(mapperClassName);
            MultipartOutputInjectionTarget mapper = (MultipartOutputInjectionTarget) mapperClass
