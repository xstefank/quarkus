...
import io.narayana.lra.client.internal.proxy.nonjaxrs.LRAParticipantProcessor;
...
import io.quarkus.narayana.lra.runtime.LRAParticipantRegistryCreator;
import io.quarkus.narayana.lra.runtime.RestClientBase;
import io.quarkus.narayana.lra.runtime.RestClientRecorder;
...
public class NarayanaLRAProcessor {
    private static final Logger log = Logger.getLogger(NarayanaLRAProcessor.class);
    ...
    void syntheticBean(BeanRegistrationPhaseBuildItem beanRegistrationPhase,
        ...
        Set<String> beanNames = beanRegistrationPhase.getContext().get(BuildExtension.Key.BEANS)
                .stream().map(bi -> bi.asBean().getBeanClass().toString()).collect(Collectors.toSet());

        Map<String, LRAParticipant> participants = new LRAParticipantProcessor().process(index, beanNames);

        participants.values()
                .forEach(p -> reflectiveClass.produce(new ReflectiveClassBuildItem(true, false, p.getJavaClass())));

        LRAParticipantRegistryCreator.participants = participants;

        beanConfigurators.produce(new BeanConfiguratorBuildItem(beanRegistrationPhase.getContext().configure(
                LRAParticipantRegistry.class)
                .types(LRAParticipantRegistry.class)
                .creator(LRAParticipantRegistryCreator.class)));

        //        beanRegistrar.produce(new BeanRegistrarBuildItem(new BeanRegistrar() {
        //
        //            @Override
        //            public void register(RegistrationContext registrationContext) {
        //                registrationContext.configure(LRAParticipantRegistry.class)
        //                        .types(LRAParticipantRegistry.class)
        //                        .creator(mc -> new LRAParticipantRegistry(participants)).done();
        //            }
        //        }));
    }

    // copied from RestClientProcessor
    ...
    ExtensionSslNativeSupportBuildItem activateSslNativeSupport() {
        return new ExtensionSslNativeSupportBuildItem(Feature.NARAYANA_LRA);
    }
